...
This means every TIMPAL wallet is quantum-resistant from the moment it is created. A quantum computer cannot derive a private key from a public key, a wallet address, or anything broadcast on-chain. The private key never leaves the device. All transactions, VRF block signatures, attestations, block signatures, and push authentication are signed with Dilithium3 and verifiable against both quantum and classical attacks.
...
### 3.5 Deterministic Single-Winner VRF Lottery

Every 10 seconds, one node wins 1.0575 TMPL. The lottery uses a deterministic scoring function seeded by the previous block's hash to ensure fairness and verifiability at any network size.

**Winner selection.** At the start of each slot, every node independently determines the winner using:

```
score(device_id) = sha256(f"{device_id}:{prev_block_hash}:{slot}")
winner = min(mature_identities, key=score)
```

Selection requires the previous block's hash, which cannot be known until that block arrives. This makes the winner for slot N unpredictable before slot N-1 is final. An attacker controlling K of N identities wins exactly K/N slots on average — this cannot be improved without controlling the previous block hash itself.

**Block construction.** The winning node builds a block containing `winner_id`, `reward_amount`, pending transactions (up to 500), pending identity registrations (up to 10), and the `prev_hash`. The winning node then signs the entire canonical block with its Dilithium3 key (`block_sig`). The block is added to the local chain and broadcast to all peers.

**Verification.** Every node that receives a block independently verifies:

1. `winner_id == min(mature_identities, key=lambda did: sha256(f"{did}:{prev_hash}:{slot}"))`
2. `winner_id` derives correctly from `public_key` (and `genesis_block_hash` for post-genesis wallets)
3. `Dilithium3.verify(public_key, canonical_block_without_sig, block_sig)` passes

All three checks must pass. A block failing any check is rejected outright.
...
**Normal extension.** When a node receives a new block, it validates all three winner rules, checks that `prev_hash` matches the current chain tip, and appends the block. This is the common case.
...
2. Walk forward from that fork point, performing full cryptographic verification on every incoming block sequentially — winner verification, block signature, all transaction signatures, chain linkage, slot ordering, supply cap.
...
- Nodes earn through the deterministic lottery: 1.0575 TMPL every 10 seconds
...
2. **Deterministic single-winner selection** — every slot selects exactly one winner via `sha256(device_id + prev_block_hash + slot)`. An attacker adding K devices to a network of N earns K/(N+K) of rewards — proportional to their share, no amplification possible.
...
Dilithium3 protects all signatures — transactions, VRF block signatures, attestations, identity registrations, and push authentication — against both classical and quantum computer attacks.

### 6.4 VRF Integrity

Every block's winner is independently verifiable by every node using `min(mature_identities, key=lambda did: sha256(f"{did}:{prev_hash}:{slot}"))`. A block claiming the wrong winner is rejected outright.
...
Building a heavier chain requires producing blocks at a faster rate than the honest network. Since there is no proof-of-work, the relevant resource is eligible identities. The honest network collectively wins slots at a faster rate than any single attacker with a realistic number of devices. Reorg paths require full cryptographic verification of every block in the fork — unsigned or improperly signed blocks are rejected at the reorg layer, not just at block acceptance.
...
The deterministic lottery ensures fairness whether the network has 10 nodes or 10 million. A single winner per slot is selected via `sha256(device_id + prev_block_hash + slot)` — unpredictable until the previous block arrives, instantly verifiable by every node. Attestation-based finality provides cryptographic — not probabilistic — irreversibility, backed by Dilithium3 signatures from a supermajority of registered identities. On-chain identity registration with consensus-enforced maturation prevents instant identity activation. Independent checkpoint balance verification means no node can corrupt the ledger's balance history without detection. Intra-block double-spend tracking closes the multi-transaction balance drain attack. The two-era model ensures the network is self-sustaining forever — first through the lottery and fees, then through fees alone.
